const CACHE='minhafrequencia-v1.4.2';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(caches.match(req).then(hit=>{
    const network=fetch(req).then(resp=>{const copy=resp.clone();caches.open(CACHE).then(c=>c.put(req,copy));return resp}).catch(()=>hit);
    return hit||network;
  }));
});